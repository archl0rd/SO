/* CARLOS EDUARDO LINO
   UNIVERSIDADE PRESIDENTE ANT�NIO CARLOS
   SISTEMAS DE INFORMA��O 6� per�odo
*/
...
/* Declara��es de Prot�tipo de fun��o */
struct processos *init_processos (int id, int prioridade, int surto);
...
int main (void) {
    ...
    plist       = init_processos(1, 2, 6);
    plist->prox = init_processos(2, 1, 7); ptmp = plist->prox;
    ptmp->prox  = init_processos(3, 3, 8); ptmp = ptmp->prox;
    ptmp->prox  = init_processos(4, 4, 9); ptmp = ptmp->prox;
    ptmp->prox  = init_processos(5, 5, 10);
    /* Simula��es executadas*/
    listprocs(plist);
    //fcfs(plist);
    //sjf(plist);
    //prioridade(plist);
...
/* � o primeiro a sair, ou seja, ser� executado primeiro */
void fcfs (struct processos *proc) {
